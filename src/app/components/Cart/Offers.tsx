...
const explanation: { Basic: dataItem[], Preceptor: dataItem[], AdVantage: dataItem[], LEGO: dataItem[] } = {
    Basic: [
        {
            heading: 'Логотип компанії на банері та на сайті проєкту',
            details: "Логотип компанії на банері та на сайті проєкту —  розміщення логотипа компанії-партнера на бренд-волл проєкту, а також на сайті Хакатону."
        },
        {
    ...
    return (
        ...
        <div className={name === 'Basic' ? 'w-auto' : 'flex'}>
            <div
                ...
                className={`cursor-pointer rounded-3xl backdrop-blur-lg min-w-[300px] sm:min-w-[400px] md:min-w-[580px] mx-auto bg-black bg-opacity-30 flex flex-col border-grey justify-between gap-3 md:gap-5 p-7 ${active ? 'border-hack-green border-2' : ''}`}>
                <div className="flex w-full justify-between items-start my-3 max-w-xl">
                    <h5 className={`${press_start.className} text-2xl md:text-4xl self-center`}>{name}</h5>
                    <button className={`${press_start.className} ${compulsory ? "cursor-not-allowed" : 'cursor-auto'} text-xl md:text-xl bg-black rounded-xl border-2 w-fit border-hack-green text-hack-green px-10 py-2 self-center`}>{`${price}$`}</button>
                ...
                <div className="flex flex-row justify-between items-end md:gap-4 gap-1">
                        {annotation && <span className="text-white bg-black border border-white text-ms md:text-md md:py-3 py-1.5 md:px-4 px-2 rounded-xl h-fit">{annotation}</span>}
                        <button onClick={() => {
                            handleModal({ name, modalData: explanation[name] as any });
                        }}>
                            <Image src="/info.svg" width={50} height={50} alt="info" className="md:w-12 w-8" />
                </button>
                </div>
...
export default function Offers() {
    ...
    return (
        <section className="min-h-screen flex flex-col items-start justify-center px-6 mx-5 w-full gap-8 my-10" id="offers">
            <h2 className={`${press_start.className} text-3xl text-hack-green my-16  md:text-5xl text-center self-center`}>Пропозиції</h2>
