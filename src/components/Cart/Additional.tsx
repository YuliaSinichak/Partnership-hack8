...
const Option = ({ name, price, active }: iOptional) => {
  ...
  return (
    <div
      ...
      className={`flex w-full rounded-full justify-between items-center md:py-4 py-2 px-2 sm:px-4 md:px-8 bg-bec-svg ${
        active ? "border-bec-darker border-2" : ""
      }`}
    >
      <span className={`text-2xl`}>{name}</span>
      <div className="right flex gap-2 md:gap-4 px-8 align-middle">
        <button
          className={`${press_start.className} text-base md:text-md text-hack-green rounded-lg border-2 w-24 border-hack-green py-2 self-center`}
        >
          {`$${price}`}
        </button>
        ...
        >
          <Image
            ...
            height={50}
            alt="info"
            className="md:w-12 w-8 md:h-12 min-h-8 hidden sm:block"
          />
        </button>
      </div>
    </div>
